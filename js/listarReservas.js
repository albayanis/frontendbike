...
function listar() {
    $.ajax({
        ...
        url: "http://129.151.113.72:8080/api/Reservation/all",
...
function listarRespuesta(items) {
    ...
    for (var i=0; i < items.length; i++) {
        ...
        */
        var texto = `Fecha inicio: ${items[i].startDate}</br>Fecha Fin: ${items[i].devolutionDate}</br>Cliente: ${items[i].client.name}</br>Bicicleta: ${items[i].bike.name}`;
        tabla +=`<tr>
