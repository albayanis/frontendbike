 ...
 */
 function editarRegistro(llaveRegistro) {
    ...
    $.ajax({
        ...
        url: "http://129.151.113.72:8080/api/Reservation/" + llaveRegistro,
...
function editarRespuesta(items) {
    ...
    let bike = "<strong>Información Biclicleta</strong><br>Marca" + items.bike.brand + "<br>Nombre: " + items.bike.name + "<br>Descripción: " + items.bike.description
...
function actualizar() {
    ...
    if (validarEditar()) {
        $.ajax({
            // la URL para la petición (url: "url al recurso o endpoint")
            url: "http://129.151.113.72:8080/api/Reservation/update",
